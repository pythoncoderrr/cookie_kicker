...
class MyTCPHandler(socketserver.BaseRequestHandler):
    ...
    def setup(self):
        ...
        my_socket_server.connected = True

    def handle(self):
        # self.request is the TCP socket connected to the client
        while True:
            if my_socket_server.kick:
                now = time.time()
                print(now)
                print(my_socket_server.kick_time)
                if (now - my_socket_server.kick_time) < 3:
                    self.request.send(b'KICK')
                    my_socket_server.kick = False

    def finish(self):
        print('Client Disconnected')
        my_socket_server.connected = False
...
<html>
  ...
  <body>
    ...
    Hi. Welcome to Ronaldinho Kicker&trade; (Patent Pending).<p>
    <div>
      Connected to the cookie machine: %s<p>
    </div>
    <div id='kickable'>
      Click
    ...
    def do_GET(self):
        ...
        if len(path_split) == 1:
            ...
            if KickingManager.check(current_user):
                ...
                self.send_header('Content-type','text/html')
                self.end_headers() 
                self.wfile.write(bytes(kick_main%(current_user,current_user,my_socket_server.connected), "utf8"))
                return
